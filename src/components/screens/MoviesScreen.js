...
const MoviesScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchMovies = async () => {
            axios(`${BASE_MOVIES_URL}${selectedMovieFilter}?api_key=${API_KEY}`)
            .then(({ data }) => {
    ...
    return (
        <View>
            ...
            <Box px={4}>
                {moviesOfType && <MoviesContainer navigation={navigation} selectedMovieFilter={selectedMovieFilter} moviesOfType={moviesOfType} />}
            </Box>
        </View>
    )
}
