...
const TVShowsScreen = ({ navigation }) => {
    ...
    useEffect(() => {
        const fetchTVShows = async () => {
            axios(`${BASE_TVSHOWS_URL}${selectedTVShowFilter}?api_key=${API_KEY}`)
            .then(({ data }) => {
    ...
    return (
        <Box>
            ...
            {TVShowsOfType && <TVShowsContainer navigation={navigation} selectedTVShowFilter={selectedTVShowFilter} TVShowsOfType={TVShowsOfType} />}
        </Box>
    )
}
